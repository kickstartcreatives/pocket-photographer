...
async function finalUpdate() {
  console.log('Starting final database update...\n');

  try {
    // Step 1: Delete existing rows
    console.log('Step 1: Deleting existing rows...');
    ...
    console.log('Step 3: Inserting new data...');
    ...
    // Step 4: Validation
    console.log('Step 4: Running validation queries...\n');
    ...
    console.log('\n🔍 Spot check (first 5 rows alphabetically):');
    spotCheck.forEach((term, i) => {
      console.log(`  ${i + 1}. ${term.element} (${term.category}) ${term.intent_tags ? '[' + term.intent_tags + ']' : '[no tags]'}`);
    });
